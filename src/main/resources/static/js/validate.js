var validate = new function() {
	...
	this.checkMandatory = function checkMandatory(qid, minusOneIsEmpty) {
		...
		if (val === '') {
			...
			if (currentError === undefined
					|| currentError.trim() !== valMandatory) {
	...
	this.currencyCheck = function currencyCheck(qid, currencySymbol,
			decimalSymbol) {
		var selector = '#' + qid;
		var val = $(selector).val();
		if (val !== ''
				&& wiz.removeCurrencyFormat($(selector).val(), currencySymbol,
						decimalSymbol) === '') {
	...
	this.setFail = function setFail(selector, message, messageSelector) {
		...
		if (message !== undefined) {
			...
			$(
					'<span class="wrn" id="' + errorId + '" data-focus="' + qid
							+ '">' + message + '</span>').insertAfter(
					messageSelector);
	...
	this.getNextInterval = function getNextInterval(intervalOrder,
			currentInterval) {
	...
	this.isPendingInterval = function isPendingInterval(intervalOrder,
	...
	this.intervalKeyUp = function intervalKeyUp(e, qid, intervalOrder, interval) {
		if ((e.keyCode >= '48' && e.keyCode <= '57')
			...
			var nextInterval = this.intervalFormat(qid, intervalOrder,
					interval, false);
	...
	this.intervalFormat = function intervalFormat(qid, intervalOrder, interval,
			isBlur) {
		...
		if ($.isNumeric(val)) {
			...
			if (interval !== 'yyyy') {
				...
				if (val.length === 2
						&& (val >= tenBound && val <= max || val.indexOf('0') === 0)) {
					if (val.indexOf('0') === 0
							&& $.inArray(interval, intervalOrder) === -1) {
				...
				if (val.length === 4) {
					...
					if (isBlur) {
						$(selector).val(
								this.convertTwoDigitYear(currentYearFourDigit,
										val));
					} else {
						var centuries = [
								currentYearFourDigit.toString().substr(0, 2),
								(currentYearFourDigit - 100).toString().substr(
										0, 2),
								(currentYearFourDigit + 100).toString().substr(
										0, 2) ];
						if ($.inArray(val, centuries) === -1) {
							$(selector).val(
									this.convertTwoDigitYear(
											currentYearFourDigit, val));
							moveNext = true;
						}
					}
				}
	...
	this.convertTwoDigitYear = function convertTwoDigitYear(
			currentYearFourDigit, twoDigitYear) {
		...
		var currentYearTwoDigit = parseInt(currentYearFourDigit.toString()
				.substr(2, 2));
	...
	this.dateIntervalValidate = function dateIntervalValidate(qid, interval,
			intervalOrder, isMandatory, year, month, day) {
		var validYear = true;
		var validMonth = true;
		var validDay = true;
		var selector = '#' + qid;
		...
		if (validYear && validMonth && validDay) {
			...
			if (interval !== undefined) {
				if (!validYear
						&& this.isPendingInterval(intervalOrder, interval,
								'yyyy')) {
					this.setDateValidation(qid, 'yyyy', year, isMandatory);
				}
				if (!validMonth
						&& this
								.isPendingInterval(intervalOrder, interval,
										'MM')) {
				...
				this.setDateValidation(qid, undefined, year + month + day,
						isMandatory);
			}
	...
	this.setDateValidation = function setDateValidation(qid, interval, value,
			isMandatory) {
		var baseSelector = '#' + qid;
		var fieldSelector = baseSelector
				+ (interval === undefined ? '' : '_' + interval);
		if (value !== '') {
			if (interval !== undefined) {
	...
	this.dateStringValidate = function dateStringValidate(qid, intervalOrder,
			isMandatory) {
		var selector = '#' + qid;
		...
		if (dateSplit.length === 3) {
			...
			if ($.isNumeric(year) && year.length === 2) {
				...
				if (separators.length === 2 && separators[0] === separators[1]) {
					year = this.convertTwoDigitYear(new Date().getFullYear(),
							year);
					var separator = separators[0];
					var format = intervalOrder.map(function(x) {
						return x.substr(0, 1);
					}).join(separator);
			...
			return this.dateIntervalValidate(qid, undefined, intervalOrder,
					isMandatory, year, month, day);
		}
		if (dateSplit.length === 1) {
			...
			for (var i = 0; i < intervalOrder.length; i++) {
				if (intervalOrder[i].indexOf('M') === 0) {
					return this.dateIntervalValidate(qid, undefined,
							intervalOrder, isMandatory, fullYear, dateSplit[0],
							dateSplit[1]);
				} else if (intervalOrder[i].indexOf('d') === 0) {
					return this.dateIntervalValidate(qid, undefined,
							intervalOrder, isMandatory, fullYear, dateSplit[1],
							dateSplit[0]);
				}
			}
		} else {
